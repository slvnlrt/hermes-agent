...
Resolution order: `delegation.base_url` (direct endpoint) takes precedence, then `delegation.provider` (full credential bundle resolved via the runtime provider system), and when neither is set children inherit the parent's provider and credentials; `delegation.model` applies in all cases, and when it is empty children inherit the parent's model. Setting `delegation.provider` alongside `delegation.base_url` keeps the explicit endpoint and carries that provider's request overrides into the child. An explicit `delegation.request_overrides` dict is honored on every branch and merges over those runtime-derived values (see [Configuration](#configuration) below).

Each `tasks[]` entry may set optional `provider` and/or `model` to pin that child only. A **provider pin** resolves a coherent credential bundle (provider, key, endpoint, wire mode) through the same runtime-provider system as CLI/config — including custom `providers:` table names — and never inherits incompatible credentials or endpoints from the batch. A **model-only pin** keeps the resolved batch account, endpoint, and command while changing the model and its model-dependent transport policy. Tasks that omit both retain the effective batch route and output cap; a pinned task lets its target transport select the applicable cap. Omit a pin or use a blank string to inherit it; every present non-string value, including JSON `null`, is rejected. An unknown provider fails the entire spawn instead of silently inheriting. All task routes are validated before any child is constructed.
